...
class TestButterworthWavelet:
    def test_butter_wav_default(self):
    ...
    def test_butter_wav_custom_parameters(self):
    ...
    def test_butter_wav_even_sample_warning(self):
...
class TestOrmsbyWavelet:
    def test_ormsby_default(self):
        t, wav = getOrmsby()
        assert isinstance(t, np.ndarray)
        assert isinstance(wav, np.ndarray)
        assert t.shape == wav.shape
    ...
    def test_ormsby_custom_frequencies(self):
    ...
    def test_ormsby_invalid_frequency_length(self):
        with pytest.raises(AssertionError):
            getOrmsby(f=(5.0, 15.0, 30.0), Samples=71, Dt=4)

    def test_ormsby_even_sample_warning(self):
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter('always')
    ...
    def test_tcrop_odd_length(self):
        t = np.arange(7) * 0.004
        cropped = _tcrop(t)
        assert cropped.shape == t.shape
        assert np.array_equal(cropped, t)

    def test_tcrop_even_length(self):
        t = np.arange(8) * 0.004
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter('always')
            cropped = _tcrop(t)
...
class TestRickerWavelet:
    ...
    def test_ricker_custom_parameters(self):
    ...
    def test_ricker_even_sample_warning(self):
        with warnings.catch_warnings(record=True) as w:
            warnings.simplefilter('always')
